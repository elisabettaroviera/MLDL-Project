...
if __name__ == "__main__":
    ...
    # Tutte le combinazioni di 4chiavi, ordinate internamente e globalmente
    all_combs = sorted([tuple(sorted(c)) for c in combinations(dict_augs_w.keys(), 4)])
        # Combo da mettere in fondo (ordine personalizzato)

    for comb in all_combs:
        # Genera il nome del progetto con la tripla di augmentations
        project_name = f"3b_GTA5_to_CITY_augmented_weather_{comb[0]}_{comb[1]}_{comb[2]}_{comb[3]}_25_percentage"
        ...
        # Ottieni gli ID del run in base al nome progetto corrente
