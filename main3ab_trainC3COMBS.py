...
"""
    ...
    # Tutte le chiavi
    all_keys = list(dict_augs_w.keys())    # Tutte le combinazioni di 3 chiavi
    all_combs_3 = sorted([tuple(sorted(c)) for c in combinations(all_keys, 3)])

    # Dividiamo in 3 gruppi quasi uguali
    group_size = (len(all_combs_3) + 2) // 3  # divide in 3 gruppi il più uniformemente possibile
    # GROUP C
    group = all_combs_3[2*group_size:]
    ...
    for comb in group:
        ...
        project_name = f"3b_GTA5_to_CITY_augmented_weather_{sorted_keys[0]}_{sorted_keys[1]}_{sorted_keys[2]}_25_percentage"
