...
def train_pidnet(epoch, old_model, dataloader_train, criterion, optimizer, iteration, learning_rate, num_classes, max_iter): # criterion == loss function
    ...
    for batch_idx, (inputs, targets, file_names) in enumerate(dataloader_train): 
        ...
        #lr = poly_lr_scheduler(optimizer, init_lr=learning_rate, iter=iteration, lr_decay_iter=1, max_iter=max_iter, power=0.3)
        #lr = 0.01
        ...
        lr_switch = learning_rate * (1 - 21 / 50) ** 0.9
        # Normalizzo tempo da 0 a 1 tra epoch_switch e T
        progress = (epoch - 21) / (50 - 21)
        lr = lr_switch * (1 - progress) ** 0.3
