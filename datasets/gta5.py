...
import numpy as np
from datasets.transform_datasets import augmentation_transform, augmentation_transform_oneof, augmentation_transform_oneof_col3_wea
...
class GTA5(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augmentation:
            augmented = augmentation_transform_oneof_col3_wea(image=np.array(image), mask=np.array(label)) # one of 2 best color and best weather
